use nom::{branch::alt, bytes::complete::tag, character::complete::{anychar, char, digit1}, combinator::map_res, multi::{many1, many_till}, sequence::separated_pair, IResult};
...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    let result = parsed.iter().fold(0, |acc, x| {
        match x {
            T::Mult(a, b) => acc + (a * b),
            _ => acc
        }
...
pub fn part_two(input: &str) -> Option<u32> {
    let (_, result) = parse_input_with_conditionals(input).iter().fold((true, 0), |(enabled, acc), x| {
        match x {
            T::Enable => (true, acc),
            T::Disable => (false, acc),
